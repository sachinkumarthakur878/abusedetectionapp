...
function Typesend({ replyMessage, onCancelReply }) {
  ...
  const handleImageSelect = async (e) => {
    ...
    try {
      // ✅ Image ko actual AI se scan karo
      const model = await loadNSFWModel();

      // Image element banao scan ke liye
      const img = new Image();
      img.src = previewUrl;
      ...
      if (porn > 0.5 || hentai > 0.5 || totalNSFW > 0.7) {
        // ✅ NSFW content detected — block karo
        setImageError(
          `🚫 Inappropriate image detected and blocked! (${Math.round(totalNSFW * 100)}% unsafe content)`
        );
        setImageFile(null);
      ...
      // Model fail hone par allow karo — degraded mode
      setImageError("");
    } finally {
