...
function VideoCall({ callType = "video", onClose, incomingCall }) {
  ...
  const cleanedUpRef = useRef(false); // FIX: prevent double-cleanup crash
  ...
  // FIX: receiverId — incoming call mein 'from' field hota hai caller ka ID
  ...
  const cleanup = useCallback(() => {
    // FIX: Double cleanup se crash hota tha — guard laga do
    ...
    // FIX: video elements ka srcObject clear karo — warna browser stream hold karta hai
  ...
  const getLocalStream = useCallback(async () => {
    try {
      ...
      // FIX: Permission denied — ek clear error dikhao aur call band karo
      alert("❌ Camera/Microphone access denied.\nBrowser settings mein permission allow karo.");
      cleanup();
      return null;
    }
  }, [callType, cleanup]);
  ...
  const createPC = useCallback((stream) => {
    // FIX: Agar purana PC exists hai to pehle close karo
    if (pcRef.current) {
      pcRef.current.close();
      pcRef.current = null;
    }
    ...
    // FIX: Connection state monitor karo — agar failed ho to user ko batao
  ...
  }, [getLocalStream, createPC, socket, incomingCall, cleanup]);

  // ── End call ──────────────────────────────────────────────
  const endCall = useCallback(() => {
    ringtoneRef.current?.stop();
    // FIX: socket null check — agar socket disconnect ho gaya ho
    if (socket && receiverId) {
      socket.emit("endCall", { to: receiverId });
    }
    cleanup();
  }, [socket, receiverId, cleanup]);
  ...
  useEffect(() => {
    ...
    // FIX: callEnded event — receiver side par bhi cleanup hona chahiye
    const onCallEnded = () => {
      ringtoneRef.current?.stop();
      cleanup();
    };
    ...
    <div className="fixed inset-0 z-50 bg-black/95 flex flex-col items-center justify-center">
      ...
      {callType === "voice" && (
        <div className="flex flex-col items-center mb-8">
          ...
          {/* FIX: audio elements ke liye ref — video call mein video refs use kiye the */}
